...
export interface Employee {
  ...
  baseSalary?: number; // Changed to number for consistency
  isActive?: boolean; // Optional based on API data
  createdAt?: string; // Optional based on API data
  updatedAt?: string; // Optional based on API data
  address?: string; // Địa chỉ
  resignationDate?: string; // Ngày nghỉ việc
  role?: Role; // Thêm role để lấy thông tin vai trò đầy đủ
}
...
export interface CreateEmployeePayload {
  ...
  baseSalary?: number; // Changed to number
...
export const EmployeeService = {
  ...
  updateEmployee: async (
    id: number,
    employeeData: Partial<CreateEmployeePayload>
  ): Promise<Employee> => {
    // Sử dụng axiosInstance
  ...
  updateEmployeeBaseSalary: async (
    id: number,
    baseSalary: number
  ): Promise<Employee> => {
    // Sử dụng updateEmployee với chỉ trường baseSalary
    return await EmployeeService.updateEmployee(id, { baseSalary: baseSalary });
  },
